...
- `hero_scaling()` — per-hero-per-level reference frame (base_health, spirit_power, required_souls) for joining against snapshot `level`
- `skill_rating(column)` — maps a badge level column to labels ("Emissary 4")
- `stale_hero_matches()` — patch-drift tripwire: matches whose recorded max_health the current base stats can't explain
