...
- `--damage` — swaps the interval columns for damage to heroes by source, reproducing the in-game damage graph tooltip, plus a Gun/Abilities/Items(gun|spirit) group block from the delivery column, then the same damage split per enemy (one row per enemy hero, interval columns)
...
uv run deadlock healing --hero Mirage [--days N] [--since 2026-07-01]
                                          ...
                                          # source table over your games of the hero, per
                                          # game lines show Abil/Items/Self % where Self is
                                          # the share that landed on the healer; totals
                                          # reconcile exactly with the scoreboard
                                          # player_healing and the self/teammate split with
                                          # self_healing/teammate_healing in stats; same
                                          # flags as damage
...
- `damage_by_source("Mirage", accounts=, matches=)` — whole-game totals per damage source across your games of a hero (one row per gun/ability/item source with `games`, `total`, `per_min`, `percent` of hero damage), the aggregate counterpart to the per-interval `deadlock match --damage`. `per_min` on gun/ability rows divides by all match minutes; item proc rows divide by minutes OWNED (item_events buy→sell/consume/end windows, so late buys read as true uptime dps; null when the source class maps to no known item). `matches=` scopes to specific match ids (one game, like `--match`). Eager frame, raises if you never played the hero; `stat="healing"` swaps the figure (backs `deadlock healing`), same for any hero_damage stat
- `damage_game_records("Mirage", days=, since=)` — one row per game of a hero with the hero damage split by delivery (`total`, `gun`, `abilities`, `items` = gun+spirit procs, `gun_pct`/`abilities_pct`/`items_pct` shares, plus day/won/K/D/A), sorted oldest first. Backs `deadlock damage`, which feeds its match ids into `damage_by_source` so both tables cover the same games (the CLI caps only the printed per-game table at 10, the frame is never capped). Fuzzy hero names resolve, raises on an unknown hero or zero games
- `healing_game_records("Mirage", days=, since=)` — the healing twin: one row per game with `total`, `abilities`, `items`, and `self` (healing that landed on the healer) plus the matching `_pct` shares. Sums reconcile exactly with `player_healing` in stats, and self vs the rest with `self_healing`/`teammate_healing`. Backs `deadlock healing`, same shape and raises as damage_game_records
