...
deadlock winrate [--days N] [--since 2026-07-01] [--by week] [--hero Mirage] [--min-rating Oracle] [--street-brawl|--private-lobby]
...
- `record_games(accounts=None, tz=None, days=None, since=None, hero=None)` — one row per match in the winrate window (days/since/hero filters applied), so its `wins` counts matches where my_games' counts account-games. daily_record, abandon_record, and unscored_record all accept a collected result via `games=` so one scan feeds all three — the winrate command does this. `summarize(record_games, ..., lf=games.lazy())` reuses that same frame
...
Current asset labels are read-time fields, not parquet columns: `players.hero` comes from `hero_id`; damage `source_name` from `source_class`; `item_events.imbued_ability` from `imbued_ability_id`; stack class/name from `ability_id`; and accolade stat name from `accolade_id`. Use the helpers above rather than joining or persisting these yourself. Historical item `item`/cost/tier/slot stay materialized on purpose.
