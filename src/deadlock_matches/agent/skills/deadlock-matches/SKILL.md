...
- `my_games(accounts=None, tz=None)` — the metric view behind summarize: players filtered to the config accounts, joined to matches, with `start_local`/`day` already in your timezone. Returns a `MetricView`, so pass it to `summarize()` or to `view_frame()` for rows. One row per match AND account, so two of your accounts in one lobby are two rows
- `record_games(accounts=None, tz=None, days=None, since=None, hero=None)` — the other metric view, one row per match in the winrate window (days/since/hero filters applied), so its `wins` counts matches where my_games' counts account-games. daily_record, abandon_record, and unscored_record all accept a collected result via `games=` so one scan feeds all three — the winrate command does this. `summarize(record_games, ..., lf=games.lazy())` reuses that same frame
- `damage_source_games(hero, stat="damage")` — one lazy row per match/account/source after duplicate total rows and non-hero targets are removed and delivery is resolved. `stat=` also covers `healing`, `heal_prevented`, and the other damage-table stats. Summarize by `source_name`, `delivery`, `day`, or account with `total`, `games`, `gun`, `abilities`, `items`, and `self`
...
- `compare_intervals(games, stat, interval_s=300)` — interval-gain facts as a metric view. Summarize by `interval` with `median_gain`, `mean_gain`, `total_gain`, and contributing `games`; the compare report uses the median measure
